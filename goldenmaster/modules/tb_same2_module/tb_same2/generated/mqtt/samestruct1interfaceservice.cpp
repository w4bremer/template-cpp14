...
namespace {
    std::map<std::string, ApiGear::MQTT::CallbackFunction> createTopicMap(SameStruct1InterfaceService* service)
    {
        return {
            ...
            {std::string("tb.same2/SameStruct1Interface/rpc/func1"), [service](const std::string&, const std::string& args, const std::string& responseTopic, const std::string& correlationData) { service->onInvokeFunc1(args, responseTopic, correlationData); } },
        };
    };
}
